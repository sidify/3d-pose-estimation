...
tf.app.flags.DEFINE_integer("epochs", 200, "How many epochs we should train for") #200
...
tf.app.flags.DEFINE_boolean("use_sh", False, "Use 2d pose predictions from StackedHourglass") #False
...
tf.app.flags.DEFINE_boolean("evaluateActionWise",False, "The dataset to use either h36m or heva") #False
...
tf.app.flags.DEFINE_string("test_dir", "", "Test directory by default is train directory. By default it is train dir, unless specified.")

# Train or load
tf.app.flags.DEFINE_boolean("test", False, "Set to True for sampling.") #added newly
tf.app.flags.DEFINE_boolean("sample", False, "Set to True for sampling.") #False
tf.app.flags.DEFINE_boolean("use_cpu", False, "Whether to use the CPU")
tf.app.flags.DEFINE_integer("load", 0, "Try to load a previous checkpoint.")

#Train data augmentation
tf.app.flags.DEFINE_boolean("augment_all", False, "Train with augmented data.")
tf.app.flags.DEFINE_boolean("augment_rot", False, "Train with augmented rotated 3d poses data.")
tf.app.flags.DEFINE_boolean("augment_flip", False, "Train with augmented flipped 3d data.")
tf.app.flags.DEFINE_boolean("augment_trans", False, "Train with augmented translated 3d data.")
tf.app.flags.DEFINE_boolean("add_kinematics", False, "Train with augmented translated 3d data.")
tf.app.flags.DEFINE_integer("add_noise", 100, "Train with noisy 3d data.")
...
def train():
  ...
  train_set_3d, test_set_3d, data_mean_3d, data_std_3d, dim_to_ignore_3d, dim_to_use_3d, train_root_positions, test_root_positions, augmented3d, train_set_3d_for_noisy = data_utils.read_3d_data(
    actions, FLAGS.data_dir, FLAGS.camera_frame, rcams, FLAGS.predict_14, FLAGS.augment_all, FLAGS.augment_rot, FLAGS.augment_flip, FLAGS.augment_trans, FLAGS.add_noise, FLAGS.add_kinematics )
  ...
  if FLAGS.use_sh:
    ...
    train_set_2d, test_set_2d, data_mean_2d, data_std_2d, dim_to_ignore_2d, dim_to_use_2d, train_set_2d_for_noisy = data_utils.create_2d_data( actions, FLAGS.data_dir, rcams, augmented3d )
  print( "done reading and normalizing data." )

  #do some noisy stuff with the data.
  #steps: 1. take a 3D pose add noise.py to 3-4 joints and project it to 2D
  #2. use noisy 2D as x and actual 3d as y.
  if FLAGS.add_noise > 0 and not FLAGS.use_sh:
    #train_set_2d, data_mean_2d, data_std_2d = data_utils.perform_noisy_actions(train_set_3d, data_mean_3d, data_std_3d, dim_to_ignore_3d, rcams)
    train_set_2d, data_mean_2d, data_std_2d = noise.perform_noisy_actions(train_set_3d_for_noisy, train_set_2d_for_noisy, FLAGS.camera_frame, rcams, FLAGS.predict_14, FLAGS.add_noise)
...
def test():
  ...
  # Load 3d data and load (or create) 2d projections
  train_set_3d, test_set_3d, data_mean_3d, data_std_3d, dim_to_ignore_3d, dim_to_use_3d, train_root_positions, test_root_positions, augmented3d, train_set_3d_for_noisy = data_utils.read_3d_data(
    actions, FLAGS.data_dir, FLAGS.camera_frame, rcams, FLAGS.predict_14, FLAGS.augment_all, FLAGS.augment_rot,
    FLAGS.augment_flip, FLAGS.augment_trans, FLAGS.add_noise, FLAGS.add_kinematics)
  # Read stacked hourglass 2D predictions if use_sh, otherwise use groundtruth 2D projections
  ...
  if FLAGS.use_sh:
    ...
    train_set_2d, test_set_2d, data_mean_2d, data_std_2d, dim_to_ignore_2d, dim_to_use_2d, train_set_2d_for_noisy = data_utils.create_2d_data(
  ...
  with tf.Session(config=tf.ConfigProto( device_count = device_count, gpu_options = gpu_options )) as sess:
    # === Create the model ===
    ...
    if FLAGS.evaluateActionWise:
      ...
      for action in actions:
        ...
        act_err, _, step_time, loss = evaluate_batches(sess, model,
                                                       data_mean_3d, data_std_3d, dim_to_use_3d, dim_to_ignore_3d,
                                                       data_mean_2d, data_std_2d, dim_to_use_2d, dim_to_ignore_2d,
                                                       0, encoder_inputs, decoder_outputs)
        cum_err = cum_err + act_err
      ...
      total_err, joint_err, step_time, loss = evaluate_batches(sess, model,
                                                               data_mean_3d, data_std_3d, dim_to_use_3d, dim_to_ignore_3d,
                                                               data_mean_2d, data_std_2d, dim_to_use_2d, dim_to_ignore_2d,
                                                               0, encoder_inputs, decoder_outputs,  FLAGS.epochs)
